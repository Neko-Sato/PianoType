...
class PianoType:
  ...
  def load_layout(self, layoutfile_path=None):
    if layoutfile_path is None:
      layoutfile_path = os.path.join(os.path.dirname(__file__), "layout.json")
    with open(layoutfile_path, 'r') as file:
      self.__config = json.load(file)
  
  def save_layout(self, layoutfile_path=None):
    if layoutfile_path is None:
      layoutfile_path = os.path.join(os.path.dirname(__file__), "layout.json")
  ...
  async def mode_setting(self, midi_stream):
    try:
      while True:
        print("Waiting for the MIDI event. (Ctrl+C to exit)")
        midi_stream.startStreaming()
        event = await anext(midi_stream)
        midi_stream.stopStreaming()
        status = int(event[0]/16)*16
        if status == 176:
